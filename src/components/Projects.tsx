...
const ProjectCard = memo(({ project, onClick }: { project: Project; onClick: () => void }) => {
  ...
  return (
    <div
      className="group bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden cursor-pointer hover-lift will-change-transform"
      onClick={onClick}
    >
      <div className="relative overflow-hidden">
        <OptimizedImage
          src={project.image}
          alt={project.title}
          className="w-full h-48 sm:h-56 md:h-64 object-cover transition-transform duration-300 group-hover:scale-105 will-change-transform"
          width={600}
          height={256}
        ...
        {/* Hover Overlay */}
        <div className="absolute inset-0 bg-gradient-to-t from-blue-900/80 to-purple-900/80 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center will-change-auto">
          <div className="text-center space-y-3 sm:space-y-4 px-4">
            <p className="text-white font-semibold text-sm sm:text-base">Click to view details</p>
            <div className="flex gap-3 sm:gap-4 justify-center">
              <button
                onClick={handleLiveClick}
                className="p-2 sm:p-3 bg-blue-600 rounded-full text-white hover:bg-blue-500 transition-all duration-200 hover:scale-110 will-change-transform"
              >
                <ExternalLink size={16} className="sm:w-5 sm:h-5" />
              </button>
              <button
                className="p-2 sm:p-3 bg-white/20 backdrop-blur-sm rounded-full text-white hover:bg-white/30 hover:scale-105 transition-all duration-200 will-change-transform"
      ...
      {/* Glow Effect */}
      <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none will-change-auto">
        <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-blue-600/15 to-purple-600/15 blur-lg"></div>
      </div>
    </div>
  );
});
...
const Projects = memo(() => {
  ...
  return (
    <>
      <section id="projects" className="py-12 sm:py-16 lg:py-20 relative overflow-hidden scroll-snap-align-start">
